...
%RK2 discretization model
%A = (I + delta_T*A_con + (delta_T^2/2!)*A_con^2)
%B = (delta_T*I + (delta_T^2/2!)*A_con)* B_con
A_rk2 = eye(n) + T*A_con + (T^2/factorial(2)*A_con^2);   %System Matrix
B_rk2 = (eye(n)*T + (T^2/factorial(2)*A_con))*B_con;     %Input Matrix
%exact model
A_ex = expm(A_con*T);
...
Q  = diag([10,1]);
R  = 1;
[Qf_rk2,~,~] = idare(A_rk2,B_rk2,Q,R,[],[]);      %RK1
% Preprocessing
Uad_rk2 = admissibleInputs(A_rk2,B_rk2,X_set,U_set,X_f,N); %RK1
% Simulation for RK2
X_log = zeros(n,steps);
U_log = zeros(m,steps);
...
x=x0;
%simulation for RK2
t_elapsed_rk2 = zeros(steps,1);
for i = 1:steps
    % Control Method: MPC
    tic
    U = MPC_opt(x,A_rk2,B_rk2,Q,Qf_rk2,R,Uad_rk2,N); % the control input sequence for predictions
    t_elapsed_rk2(i) = toc;
    u = U(1:m);                                      % the optimal control input at current state
    
    % Prediction
    [A_,B_] = liftedModel(A_rk2,B_rk2,N);
...
% legend("RK2");
% title("True continuous system closed-loop")
% %Error computation
% 
% fprintf('the error between RK2 MPC prediction states and true continuous time system states is:%0.3f\n', error);
% 
% %Compute the invariant set
% X_f_rk = ControlInvariantSet( A_rk2, B_rk2, Polyhedron('A',X_set.A,'b',X_set.b), Polyhedron('A',U_set.A,'b',U_set.b), 'maxItr',100);
...
% legend('invariant set of exact model','invariant set of RK2 model');
% xlim([-2 4]);
% ylim([-2 3]);
% title('the terminal invariant set')
% 
% %Compute the invariant set
% sys_rk = LTISystem('A', A_rk2);
